...
int process_user_modify_request(int acceptfd,MSG *msg)
{
    ...
    char *cuowuxinxi;
	char sql[DATALEN] = {0};
	char lishijilubuff[DATALEN] = {0};
	switch (msg->recvmsg[0])
	{
	case 'P':
		sprintf(sql,"update usrinfo set phone='%s' where staffno=%d;"\
				                       ,msg->info.phone,msg->info.no);
        sprintf(lishijilubuff,"%s修改工号为%d的电话为%s",\
				          msg->username,msg->info.no,msg->info.phone);
		break;

	case 'D':
		sprintf(sql,"update usrinfo set phone='%s' where staffno=%d;"\
				                       ,msg->info.phone,msg->info.no);
        sprintf(lishijilubuff,"%s修改工号为%d的家庭住址为%s",\
				          msg->username,msg->info.no,msg->info.addr);
        break;

	case 'M':
		sprintf(sql,"update usrinfo set phone='%s' where staffno=%d;"\
				                       ,msg->info.phone,msg->info.no);
        sprintf(lishijilubuff,"%s修改工号为%d的密码为%s",\
				          msg->username,msg->info.no,msg->info.passwd);
		break;
	}
	...
	if(sqlite3_exec(db,sql,NULL,NULL,&cuowuxinxi) != SQLITE_OK){
	  printf("%s.\n",cuowuxinxi);
      sprintf(msg->recvmsg,"数据修改失败\n");
	}else{
	  printf("the database is updated successfully.\n");
      sprintf(msg->recvmsg,"数据修改成功\n");
	  /*history_init(msg,lishijilubuff);*/
	}
...
int process_user_query_request(int acceptfd,MSG *msg)
{
    ...
    sprintf(sql,"请选择想要查找的用户",msg->username);
...
int process_admin_modify_request(int acceptfd,MSG *msg)
{
	...
	case 'N':
		...
		sprintf(historybuf,"%s修改工号为%d的用户名为%s",\
				msg->username,msg->info.no,msg->info.name);
		break;
	case 'A':
		sprintf(sql,"update usrinfo set age=%d where staffno=%d;",\
				msg->info.age, msg->info.no);
		sprintf(historybuf,"%s修改工号为%d的年龄为%d",\
				msg->username,msg->info.no,msg->info.age);
		break;
	case 'P':
		sprintf(sql,"update usrinfo set phone='%s' where staffno=%d;",\
				msg->info.phone,msg->info.no);
		sprintf(historybuf,"%s修改工号为%d的电话为%s",\
				msg->username,msg->info.no,msg->info.phone);
		break;
	case 'D':
		sprintf(sql,"update usrinfo set addr='%s' where staffno=%d;",\
				msg->info.addr, msg->info.no);
		sprintf(historybuf,"%s修改工号为%d的家庭住址为%s",\
				msg->username,msg->info.no,msg->info.addr);
		break;
	case 'W':
		sprintf(sql,"update usrinfo set work='%s' where staffno=%d;",\
				msg->info.work, msg->info.no);
		sprintf(historybuf,"%s修改工号为%d的职位为%s",\
				msg->username,msg->info.no,msg->info.work);
		break;
	case 'T':
		sprintf(sql,"update usrinfo set date='%s' where staffno=%d;",\
				msg->info.date, msg->info.no);
		sprintf(historybuf,"%s修改工号为%d的入职日期为%s",\
				msg->username,msg->info.no,msg->info.date);
		break;
	case 'L':
		sprintf(sql,"update usrinfo set level=%d where staffno=%d;",\
				msg->info.level, msg->info.no);
		sprintf(historybuf,"%s修改工号为%d的评级为%d",\
				msg->username,msg->info.no,msg->info.level);
		break;
	case 'S':
		sprintf(sql,"update usrinfo set salary=%.2f where staffno=%d;",\
				msg->info.salary, msg->info.no);
		sprintf(historybuf,"%s修改工号为%d的工资为%.2f",\
				msg->username,msg->info.no,msg->info.salary);
		break;
	case 'M':
		sprintf(sql,"update usrinfo set passwd='%s' where staffno=%d;",\
				msg->info.passwd, msg->info.no);
		sprintf(historybuf,"%s修改工号为%d的密码为%s",\
				msg->username,msg->info.no,msg->info.passwd);
		break;
	}
...
int process_admin_adduser_request(int acceptfd,MSG *msg)
{
	...
	sprintf(sql,"insert into usrinfo values(%d,%d,'%s','%s',%d,'%s','%s',\
		    '%s','%s',%d,%f);",\
			msg->info.no,msg->info.usertype,msg->info.name,msg->info.passwd,\
			msg->info.age,msg->info.phone,msg->info.addr,msg->info.work,\
			msg->info.date,msg->info.level,msg->info.salary);
...
int process_admin_deluser_request(int acceptfd,MSG *msg)
{
	...
	char buf[DATALEN] = {0};
	char *errmsg;
	printf(sql,"delete from usrinfo where staffno=%d and name='%s';",\
...
int process_admin_query_request(int acceptfd,MSG *msg)
{
	...
	if(msg->flags == 1){
		...
		if(sqlite3_get_table(db, sql, &resultp,&nrow,&ncolumn,&errmsg) != SQLITE_OK){
			...
			for(i = 0; i < nrow; i ++){
				printf("%s    %s     %s     %s     %s     %s     %s     %s 	  %s     %s     %s.\n",
						resultp[index+ncolumn-11],
						resultp[index+ncolumn-10],resultp[index+ncolumn-9],
						resultp[index+ncolumn-8],resultp[index+ncolumn-7],
						resultp[index+ncolumn-6],resultp[index+ncolumn-5],
						resultp[index+ncolumn-4],resultp[index+ncolumn-3],
